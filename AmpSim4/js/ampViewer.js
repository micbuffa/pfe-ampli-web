class AmpViewer {
  constructor(amp, ampName) {
    ...
    this.powerAmpHiLoCutFiltersStatus = document.querySelector(
      "#powerAmpLoHiCutFiltersStatus"
    );
...
*/
  ...
  changePowerAmpHiAndLoCutFiltersStatus(filtersEnabled) {
    this.powerAmpHiLoCutFiltersStatus.innerHTML =
      "Hi/Lo cut filters : " + (filtersEnabled ? "On" : "Off");
  }
  ...
  // Gain before preamp
  changeGainBeforePreampValue(sliderval) {
    // update output labels
    var output = document.querySelector("#gainBeforePreamp");
    output.value = parseFloat(sliderval).toFixed(1);

    // refresh slider state
    var slider = document.querySelector("#gainBeforePreampSlider");
    slider.value = parseFloat(sliderval).toFixed(3);
  }
  ...
  changeOutputGainCabinetAmp(sliderVal) {
    // update output labels
    var output = document.querySelector("#cabinetOutputGain");
    output.value = parseFloat(sliderVal).toFixed(1);

    // refresh slider state
    var slider = document.querySelector("#outputGainCabinetSlider");
    slider.value = parseFloat(sliderVal).toFixed(1);
  }
