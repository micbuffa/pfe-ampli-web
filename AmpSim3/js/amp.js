...
function Amp(context) {
    ...
    function printCurrentAmpValues() {
        var currentPresetValue = {
            ...
            boost: preamp.boost.isActivated(),

            LS1Freq: preamp.lowShelf1.frequency.value,
            LS1Gain: preamp.lowShelf1.gain.value,
            LS2Freq: preamp.lowShelf2.frequency.value,
            LS2Gain: preamp.lowShelf2.gain.value,
            gain1: preamp.preampStage1Gain.gain.value,
            distoName1 : preamp.menuDisto1.value,
            K1: preamp.getDistorsionValue(0),
            HP1Freq: preamp.highPass1.frequency.value,
            HP1Q: preamp.highPass1.Q.value,

            LS3Freq: preamp.lowShelf3.frequency.value,
            LS3Gain: preamp.lowShelf3.gain.value,
            gain2: preamp.preampStage2Gain.gain.value,
            distoName2 : preamp.menuDisto2.value,
            K2: preamp.getDistorsionValue(1),

            OG: (output.gain.value*10).toFixed(1),
            BF: ((tonestack.bassFilter.gain.value / 7) + 10).toFixed(1), // bassFilter.gain.value = (value-5) * 3;
            MF: ((tonestack.midFilter.gain.value / 4) + 5).toFixed(1), // midFilter.gain.value = (value-5) * 2;
            TF: ((tonestack.trebleFilter.gain.value / 10) + 10).toFixed(1), // trebleFilter.gain.value = (value-5) * 5;
            PF: ((tonestack.presenceFilter.gain.value / 2) + 5).toFixed(1), // presenceFilter.gain.value = (value-5) * 2;
            EQ: eq.getValues(),
